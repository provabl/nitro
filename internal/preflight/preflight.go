...
// nitro needs for its AWS-touching operations. It uses read-only
...
// principal holds nitro's required actions. Fail-closed: a config/credential
...
func check(ctx context.Context, stsSvc stsIdentityAPI, iamSvc iamSimAPI) []Result {
	...
	out, err := iamSvc.SimulatePrincipalPolicy(ctx, &iam.SimulatePrincipalPolicyInput{
		...
		ActionNames:     nitroRequiredActions,
	})
	if err != nil {
		return []Result{{
	...
	for _, ev := range out.EvaluationResults {
		...
		results = append(results, Result{
			...
			Remediation: "Grant " + action + " to the nitro principal (see required-permissions.md)",
		})
	}
	if len(results) == 0 {
		return []Result{{
			...
			Remediation: "Review required-permissions.md and the nitro principal's policy",
		}}
	}
	return results
}
